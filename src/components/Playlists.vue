  ...
  <nav class="panel is-primary">
    ...
    <template v-for="(playlist, index) in playlists">
      <p class="panel-block playlist-item" :key="playlist.slug">
        <router-link :to="'music/playlist/' + playlist.slug" class="planel-block">
          <span class="panel-icon"><fa-icon icon="book"/> </span> {{ playlist.name }}
        </router-link>
  ...
  created () {
    ...
    .then(() => {
      if (this.selectedPlaylistsSlug !== undefined) {
        this.$emit('setPlaylist', this.playlists.find(pl => pl.slug === this.selectedPlaylistsSlug))
      }
    })
  }
}
